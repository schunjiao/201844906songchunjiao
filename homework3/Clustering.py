...
datapath='D:\\CodeSet\\pycharm\\homework3\\Tweets.txt' #Tweets数据集路径
...
def GaussianMixture(tfidf_matrix, num_clusters):
    gm = GaussianMixture(n_components=num_clusters, covariance_type='diag', max_iter=20, random_state=0)
...
if __name__ == '__main__':
    ...
    '''birch算法'''
    judge_list = birch(tfidf_matrix)
    rate = evaluate(judge_list, real_list)
    print('使用birch算法进行聚类，准确率为：' + str(rate))
